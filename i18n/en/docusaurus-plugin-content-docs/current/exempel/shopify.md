...
This guide walks through how to embed the Marvify 3D viewer into a Shopify storefront. The example uses the default **Horizon** theme and shows how to replace the product image on a product page with an interactive 3D model. The same approach can be adapted to any Shopify theme that uses Liquid templates.
...
In your Shopify admin, go to **Online Store → Themes**. On your active theme, click **Customize theme**.

<p align="center">
  <img src={require('@site/static/img/shopify_1.webp').default} alt="Customize theme button in Shopify admin" style={{ maxWidth: '600px', display: 'block' }} />
</p>
...
To edit the underlying template files, close the theme editor and return to **Online Store → Themes**. Click the **three-dot menu (⋯)** next to your current theme and select **Edit code**.
...
## 3. Add the viewer script to theme.liquid

In the file tree on the left side, expand the **Layout** folder and open **theme.liquid**. This is the main layout file that wraps every page in your store — any script loaded here is available across the entire site.
...
## 4. Insert the viewer into the product media template
...
<p align="center">
  <img src={require('@site/static/img/shopify_5.webp').default} alt="product-media-gallery-content.liquid open showing where the viewer is inserted" style={{ maxWidth: '700px', display: 'block' }} />
</p>

Inside the main `{% else %}` block — the branch that renders when a product exists — add a conditional before the existing `<media-gallery>` element. This checks whether the current product is the one with the viewer, and if so renders the Marvify component instead of the standard image gallery:
...
Replace `stylish-bag` with the handle of your actual product, and `fashion-expo-01` with your own model ID from your Marvify account. The `{% else %}` branch retains the original media gallery code so that all other products continue to display their images as normal.

### Viewer attributes used in this example

| Attribute             | Value        | Effect |
| --------------------- | ------------ | ------ |
| `model-id`            | `fashion-expo-01` | Identifies which 3D model to load from Marvify. |
